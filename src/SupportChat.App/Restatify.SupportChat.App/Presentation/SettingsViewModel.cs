...
public sealed class SettingsViewModel : INotifyPropertyChanged
{
	...
	private string _selectedThemeMode = ThemeModes.System;
	private string _status = "";

	public SettingsViewModel(string? from, IUserSettingsStore userSettingsStore, INavigator navigator, IStringLocalizer localizer, ILogger<SettingsViewModel> logger)
	{
		...
		_selectedThemeMode = current.ThemeMode;
	...
	public IReadOnlyList<ThemeModeOption> AvailableThemeModes =>
	[
		new(ThemeModes.System, T("ThemeModeName_System", "System")),
		new(ThemeModes.Light, T("ThemeModeName_Light", "Light")),
		new(ThemeModes.Dark, T("ThemeModeName_Dark", "Dark"))
	];
	...
	public async Task Save()
	{
		...
		{
			SetErrorStatus(T("Error_BaseUrlRequired", "Base URL is required."), "Settings.Save.BaseUrlRequired");
			return;
		}

		if (!Uri.TryCreate(BaseUrl.Trim(), UriKind.Absolute, out var uri))
		{
			SetErrorStatus(T("Error_BaseUrlInvalid", "Base URL is invalid."), "Settings.Save.BaseUrlInvalid");
			return;
		}

		if (uri.Scheme != Uri.UriSchemeHttps && !(uri.Scheme == Uri.UriSchemeHttp && uri.Host is "127.0.0.1" or "localhost"))
		{
			SetErrorStatus(T("Error_UseHttpsOrLocalhostHttp", "Use HTTPS (HTTP only allowed for localhost)."), "Settings.Save.ProtocolValidation");
			return;
		}

		_userSettingsStore.Save(new UserSettings(BaseUrl, ApiKey, SelectedLanguageCode, LogoUrl, SelectedThemeMode));
		UserSettingsStore.ApplyLanguage(SelectedLanguageCode);
		await UserSettingsStore.ApplyTheme(SelectedThemeMode);
	...
	private void SetErrorStatus(string message, string context)
	{
